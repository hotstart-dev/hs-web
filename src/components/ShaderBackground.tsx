...
* Modified for purple/violet theme
*/
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		vec2 p=uv;
		float d=length(p);
		// Purple/violet color scheme: swap RGB channels for purple tones
		col+=.00125/d*(cos(sin(i)*vec3(2,1,3))+1.);
		float b=noise(i+p+bg*1.731);
		col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
		// Purple background tones: more blue/magenta, less green
		col=mix(col,vec3(bg*.18,bg*.08,bg*.25),d);
	}
	O=vec4(col,1);
}`;
